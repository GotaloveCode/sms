...
break;
  ...
  $ret= mysql_query($sqll) or die(mysql_error());
  if(mysql_num_rows($ret) < 1) {
    echo "No Student records to transfer";
  }
  while($rwz = mysql_fetch_assoc($ret)){
    extract($rwz);
	...
	$resultx  = mysql_query($sql) or die('Cannot get Info.');
	$gawa			=	$total_fee_paid;
	$payable		=	0;
	 while($row1  = mysql_fetch_assoc($resultx)): 
						$balance = 0;
						$paid    = 0;
 ...
 if ($balance!=0){
 	...
 	$escaped_values = array_map('mysql_real_escape_string', array_values($insertarray));
