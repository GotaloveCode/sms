...
mysql_query($query) or die ('error updating database,check your inputs1');
...
mysql_query($query) or die ('error updating database,check your inputs2');

//Sucess Message
$_SESSION['adminNo']=$adminNo;
// get school info
$sql = "SELECT * FROM school_credentials";
$result = mysql_query($sql) or die('Cannot get Info1.');
...
$result1 = mysql_query($sql) or die('Cannot get Info2.');
...
$result = mysql_query($sql) or die('Cannot get Info4.');
$row7    = mysql_fetch_assoc($result);
extract($row7);
//get receipt id
//$sql = "SELECT rctpID FROM receipts ORDER BY rctpID DESC LIMIT 1";
//$result = mysql_query($sql) or die('Cannot get Info5.');
...
$resul     = mysql_query($sql2) or die(mysql_error());
$rwz = mysql_fetch_array($resul);
