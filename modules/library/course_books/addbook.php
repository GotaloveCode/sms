...
mysql_query($query) or die ('error updating database,check your inputs(insert)');

$query="SELECT count FROM books_count WHERE subject_id='$subject' AND class_id='$class'";
$result=mysql_query($query);
if (mysql_num_rows($result) > 0) {//books already exists for this class and subject
$row=mysql_fetch_array($result);
$count=$row[0];
$count1=$count+1;

$query="UPDATE books_count SET count='$count1' WHERE subject='$subject' AND class='$class'";
mysql_query($query) or die ('error updating database,check your inputs(count update)');
?>
