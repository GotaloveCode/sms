...
class credential{//function to display school creds
function creds()
{
$sql = "Select * FROM school_credentials";
$result     = mysql_query($sql) or die(mysql_error());
$creds = mysql_fetch_assoc($result);
extract($creds);
...
$result_period=mysql_fetch_assoc(mysql_query($sql_period)) or die (mysql_error());
...
class studentProfile
{
	...
	function studentPosition($term_id,$stud_id)
	{
		...
		$result_class=mysql_fetch_assoc(mysql_query($class));
		extract($result_class);
		$total_student="SELECT COUNT( stud_id ) AS students FROM student_details where class_id='$class_id'";
		$re_total=mysql_query($total_student);
		$ro_total = mysql_fetch_assoc($re_total);
		extract($ro_total);
		...
		if($stud_a==$stud_id){$j=$j+1; $position=$j.'/'.$students;}
		 $j += 1;
		}
		}
		else{
...
class examTypeMarks 
{
	...
	function classPosition($term_id,$stud_id,$subject_id)
	{
		...
		$class_results=mysql_query($class_query) or die(mysql_error()) ;
		$row_class=mysql_fetch_assoc ($class_results);
		extract($row_class);
		...
		//$gradebk_id=$rw[2];
		$total_student="SELECT COUNT( stud_id ) AS students FROM student_details where class_id='$class_id'";
		$re_total=mysql_query($total_student) or die(mysql_error()) ;
		$ro_total = mysql_fetch_assoc($re_total);
		//echo $total_student;
		extract($ro_total);
	...
	function teacherComments($term_id,$stud_id,$subject_id)
	{
		...
		if (mysql_num_rows($result) > 0) 
		{
			$ro_total = mysql_fetch_assoc($result);
			extract($ro_total);
...
class getUpperClassMarks
{
	...
	$result=mysql_query($sql);
	if (mysql_num_rows($result) > 0) {
	$row_subject = mysql_fetch_assoc($result);
	extract($row_subject);
